...
/* findDet(matrix , order_of_matrix) */  //tim det
...
int findDetInverse(int R , int D = 26){ // R la so du hoac dinh thuc{
	int i = 0 ;
	int p[100] = {0,1};
	int q[100] = {0} ; // thuong so (quotient)
	while(R!=0){
		q[i] = D/R ;
		int oldD = D ;
		D = R ;
